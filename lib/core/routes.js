...
function routes(app) {
	...
	function initList(respectHiddenOption) {
		return function(req, res, next) {
			...
			if (!req.list || (respectHiddenOption && req.list.get('hidden')) && typeof(req.list.get('hidden')) != 'function' ) {
				debug('could not find list ', req.params.list);
				req.flash('error', 'List ' + req.params.list + ' could not be found.');
				return res.redirect('/keystone');
			} else if( req.list && req.list.get('hidden') && typeof(req.list.get('hidden')) == 'function' ) {
				if( req.list.get('hidden')(req) ) {
					debug('could not find list ', req.params.list);
					req.flash('error', 'List ' + req.params.list + ' could not be found.');
					return res.redirect('/keystone');
				}
